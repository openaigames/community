<p align="right"><b>简体中文</b> · <a href="README.en.md">English</a></p>

<p align="center"><img src="https://raw.githubusercontent.com/openaigames/.github/main/assets/banner-v2-corrected.png" alt="OpenAIGames — Make something worth playing." width="100%" /></p>

### 一个点子，可以在这里长成游戏。

这里是 **OpenAIGames 的共创空间**。带着想法来，留下能玩的 Demo；找到一起做的人，也给别人的作品一次认真的试玩。

**[带一个点子来 ↗](https://github.com/openaigames/community/issues/new?template=01-game-idea.yml)**　·　**[提交可试玩 Demo ↗](https://github.com/openaigames/community/issues/new?template=02-game-demo.yml)**　·　**[通过 PR 留存 ↗](CONTRIBUTING.md)**

[浏览点子](https://github.com/openaigames/community/issues?q=is%3Aissue%20label%3Aidea)　·　[浏览 Demo 投稿](https://github.com/openaigames/community/issues?q=is%3Aissue%20label%3Ademo)　·　[投稿索引](SHOWCASE.md)　·　[参与方式](CONTRIBUTING.md)

---

### 让创作持续下去

| 来到这里时 | 留下一点什么 | 接下来一起做 |
| --- | --- | --- |
| 有一个想法 | 核心互动、最小可玩版本、需要的帮助 | 找搭档，做第一版 |
| 有一个 Demo | 试玩链接、操作方式、最想听到的反馈 | 试玩，交流，继续迭代 |
| 遇到反复出现的问题 | 制作场景、当前步骤、关联的 Demo | 判断需求，研发自己的工具 |

想先讨论，可以开 Issue，分别使用 `idea`、`demo` 或 `tooling` 标签。已有整理好的资料，也可以直接提 PR，不必先开 Issue。

### 用 PR 把创作留在仓库里

- [ideas/](ideas/)：点子、核心玩法和共创计划。
- [demos/](demos/)：试玩入口、创作者署名、制作记录和版本变化。
- [tooling/](tooling/)：来自真实制作的工具需求与验证思路。

复制对应的 [Markdown 模板](templates/)，在目录中添加一份记录并提 PR。合并后，资料进入仓库，修改历史和 PR 讨论都能保留。后续版本更新同一份记录；有相关 Issue 时互相链接，便于追溯。

### 从真正的制作中发现工具需求

做游戏时遇到重复、费时的步骤，可以[记录一个工具需求](https://github.com/openaigames/community/issues/new?template=03-tool-need.yml)。先说清楚具体问题、关联作品和希望改善的结果，再一起决定做什么。

### 共创记录与展示网页

`community` 沉淀点子、作品投稿、试玩反馈和协作记录。正式游戏与工具的代码，在项目确定后放到各自的仓库，并链接回这里。

Demo 和游戏的展示网页正在开发中，将提供集中展示、浏览与游玩入口。这里的投稿索引用于整理作者、试玩地址、源码和制作记录。

投稿索引从真实投稿开始整理。欢迎第一个可玩的版本，不用等到它完美。

<p align="center"><b>好玩是起点。开放，让创作继续。</b><br /><sub>MAKE SOMETHING WORTH PLAYING.</sub></p>
